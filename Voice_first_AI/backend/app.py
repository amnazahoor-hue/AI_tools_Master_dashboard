...
if __name__ == "__main__":
    # Disable SocketIO/Flask reloader to avoid double-binding the same port on Windows.
    try:
        project_root = BASE_DIR.parent.parent  # C:\Portfolio
        log_path = project_root / "debug-d56b4c.log"
        with open(log_path, "a", encoding="utf-8") as f:
            f.write(
                json.dumps(
                    {
                        "sessionId": "d56b4c",
                        "id": f"voice_start_{int(time.time() * 1000)}",
                        "timestamp": int(time.time() * 1000),
                        "location": "Voice_first_AI/backend/app.py:__main__",
                        "message": "socketio.run startup config",
                        "data": {"port": 5110, "debug": False, "use_reloader": False},
                        "runId": "post-fix",
                        "hypothesisId": "H3",
                    }
                )
        ...
        # Debug logging must never prevent the app from starting.
        pass

    socketio.run(app, host="0.0.0.0", port=5110, debug=False, use_reloader=False)
